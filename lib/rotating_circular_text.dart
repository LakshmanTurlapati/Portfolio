...
class RotatingCircularText extends StatefulWidget {
  // Parameters remain the same
   ...
   final Duration? startDelay; // Added start delay parameter
   ...
   const RotatingCircularText({
     ...
     this.startDelay, // Added start delay parameter
   });
...
class _RotatingCircularTextState extends State<RotatingCircularText>
  ...
  void didUpdateWidget(RotatingCircularText oldWidget) {
    super.didUpdateWidget(oldWidget);
     ...
     if (oldWidget.startDelay != widget.startDelay) {
  ...
  Widget build(BuildContext context) {
    // Only show animation if it has started
     ...
     if (_charPainters1.isNotEmpty || _painter2.text?.toPlainText().isNotEmpty == true) {
        ...
        for (int p = 0; p < numberOfPairs; p++) {
           ...
           for (int i = 0; i < _charPainters1.length; i++) {
             ...
             final double x = center.dx + radius * math.cos(-drawAngle); // Negative angle for opposite direction
             final double y = center.dy + radius * math.sin(-drawAngle); // Negative angle for opposite direction

             recordingCanvas.save();
             recordingCanvas.translate(x, y);
             recordingCanvas.rotate(-drawAngle + math.pi / 2.0 + math.pi); // Negative angle for opposite direction
