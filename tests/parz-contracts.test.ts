...
describe('Parz persona contract', () => {
  it('anchors answer first tone, current work, flagship projects, and gap radar explanation', () => {
    expect(systemPrompt).toContain('Answer first');
    expect(systemPrompt).toContain('warm practical builder friend');
    expect(systemPrompt).toContain("Match the user's energy");
    expect(systemPrompt).toContain('Light profanity is allowed');
    expect(systemPrompt).toContain('Rude in, push back sharply');
    ...
    expect(systemPrompt).toContain('alignment and gap radar');
  ...
  it('carries the OpenClaw voice fingerprint into the prompt', () => {
    expect(systemPrompt).toContain('Voice fingerprint');
    for (const word of publicProfile.voiceFingerprint.vocabulary) {
      expect(systemPrompt).toContain(word);
    }
    for (const example of publicProfile.voiceFingerprint.selfDeprecation) {
      expect(systemPrompt).toContain(example);
    }
    for (const closer of publicProfile.voiceFingerprint.closers) {
      expect(systemPrompt).toContain(closer);
    }
    expect(systemPrompt).toContain('Never use hyphen characters in your replies');
    expect(systemPrompt).toContain('Never open with a probing question');
  });
  ...
  it('keeps protected categories and refusal boundaries in the prompt contract', () => {
    ...
    expect(systemPrompt).toContain('secrets, config, or API key');
